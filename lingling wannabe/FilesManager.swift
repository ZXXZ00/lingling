...
final class FilesManager {
    ...
    let url = URL(string: "https://linglingwannabe.com/file/upload")!
    ...
    private func uploadHelper(username: String, time: Int) {
        ...
        let json: [String: Any]
        if let lists = UserDefaults.standard.array(forKey: username+"_instruments") {
            json = ["username": username, "filename": "\(time).flac", "instruments": lists]
        } else {
            json = ["username": username, "filename": "\(time).flac"]
        }
        postJSON(url: url, json: json, token: nil, success: {
            data, res in
